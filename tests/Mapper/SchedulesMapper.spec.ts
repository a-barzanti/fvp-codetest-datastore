import {mapScheduleEvents, IScheduleEvent} from "../../src/Mapper/SchedulesMapper";
...
describe("Schedules Mapper Maps query results correctly", () => {
    it("should build a compliant Schedule Event object", () => {
        const expectedObject: IScheduleEvent[] = [
            {
        ...
        const querySampleUnmarshalled = [
            {
                ...
                gsi3sk: "crid://www.example.tv/something",
                endTime: 1550639400,
                crids: {
        ...
        const querySample: DynamoDB.AttributeMap[] = querySampleUnmarshalled.map(
            (querySampleElement: any) => {
                return DynamoDB.Converter.marshall(querySampleElement);
            },
        );
        const mappedQuery: IScheduleEvent[] = querySample.map(mapScheduleEvents);
